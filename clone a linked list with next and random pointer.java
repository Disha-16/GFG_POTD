...
class Solution {
    ...
    Node copyList(Node head) {
        ...
        // Create a hashmap to store the mapping from original nodes to new nodes
        HashMap<Node, Node> map = new HashMap<>();

        // Step 1: Create new nodes (copy) and map them with original nodes
        Node current = head;
        while (current != null) {
        ...
        while (current != null) {
            ...
            newNode.next = map.get(current.next);    // Setting next pointer
            newNode.random = map.get(current.random); // Setting random pointer
            current = current.next;
        }
